...
recherche.addEventListener("keyup", (e) =>{
    ...
    const result = utilisateurs.filter(item => item.prenom.toLocaleLowerCase().includes(input.toLocaleLowerCase()))
    console.log(result)

    if(result == ""){
        utilisateursContainer.innerHTML = `<p>Rien</p>`
    }else
    afficherUtilisateurs(result)
})
